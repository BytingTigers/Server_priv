    ...
    for (int i = 0; i < MAX_CLIENTS; ++i) {
        if (server->clients[i]) {
            ...
            if (uid == -1) { // uid is -1 if message from server
                ...
                snprintf(full_message, sizeof(full_message), "%s: %s",
                         server->clients[uid]->username, s);
            }

            if (write(server->clients[i]->sockfd, full_message,
                      strlen(full_message)) < 0) {
            ...
            if (uid >= 0) { // save only if message is sent from user
                ...
                redisReply *reply = redisCommand(redis_context, "LPUSH %s %s",
                                                 key, full_message);
                if (reply == NULL) {
...
void *handle_client(void *arg) {
    ...
    char key[32];
    snprintf(key, sizeof(key), "chatroom_%d", server->server_port);
    redisReply *reply = redisCommand(redis_context, "LRANGE %s 0 20",
                                     key); // load recent 20 messages
    if (reply == NULL) {
        sprintf(buffer, "Failed to load message from Redis\n");
        send_message(buffer, -1, server, NULL);
    } else {
        for (int element_id = reply->elements - 1; element_id >= 0;
             element_id--) {
            snprintf(buffer, sizeof(buffer), "%s",
                     reply->element[element_id]->str);
